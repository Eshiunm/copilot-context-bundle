...
Commands:
  ...
  update <targetPath>       Refresh managed items from the current bundle source.
...
Examples:
  ...
  ${CLI_NAME} update ../sandbox --bundle-source . --dry-run
  ${CLI_NAME} promote ../sandbox --file .github/instructions/storage-manager-fe/storage-manager-fe.instructions.md --bundle-source .
`;
}
...
Options:
  ...
  --force                  Replace an existing install and prune stale managed files.
...
Options:
  --detail                 Output managed item paths in text mode.
...
function getUpdateHelp() {
  return `Usage:
  ${CLI_NAME} update <targetPath> [options]

Options:
  --bundle-source <path>   Override the bundle source root for local development.
  --dry-run                Show the planned update operations without writing files.
  --force                  Overwrite drifted managed items and target-path collisions.
  --prune                  Remove managed items that no longer exist in the bundle source.
  --json                   Output structured JSON summary.
`;
}
