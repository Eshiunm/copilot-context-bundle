...
function parseDestinationOverride(destination, repoRelativePath) {
  const sourceRelativePath = stripInstructionCategoryFromTargetPath(repoRelativePath);

  if (destination === 'shared') {
    return `shared/${sourceRelativePath}`;
  }
  ...
  if (destination && destination.startsWith('profile:')) {
    ...
    return `profiles/${profileName}/${sourceRelativePath}`;
  }
...
async function runPromote(argv) {
  ...
  const previousSourceRelativePath = managedItem ? managedItem.sourcePath : null;
  ...
  const previousSourceAbsolutePath = previousSourceRelativePath
    ? path.join(bundleRoot, fromPosixPath(previousSourceRelativePath))
    : null;
  ...
  if (
    ...
    previousSourceRelativePath !== destinationRelativePath &&
    previousSourceAbsolutePath &&
    (await pathExists(previousSourceAbsolutePath))
  ) {
    await fs.rm(previousSourceAbsolutePath, { force: true });
  }
