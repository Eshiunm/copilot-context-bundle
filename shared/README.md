...
- `shared/.github/instructions/copilot-context-bundle-operations.instructions.md` 是共用操作指南，提供 `install / status / update / promote` 的判斷方式、Windows PowerShell 注意事項與推薦步驟。
- 如果後續需要更人類導向的 step-by-step 範例，可再補一份 `shared/docs/` runbook；目前先以 `.instructions.md` 同時服務 AI 與人類讀者。
